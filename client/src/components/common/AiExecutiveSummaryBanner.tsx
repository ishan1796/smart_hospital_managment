...
import { Sparkles, RefreshCw, ChevronDown, ChevronUp, Bot, FileText, CheckCircle2 } from "lucide-react";
...
export const AiExecutiveSummaryBanner: React.FC<AiExecutiveSummaryBannerProps> = ({
  role,
  defaultOpen = true,
}) => {
  ...
  const fetchSummary = async () => {
    ...
    try {
      ...
      setError("Unable to generate live Gemini AI summary at this moment.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-4 sm:p-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-xs border border-white/20 flex items-center justify-center shrink-0">
            <Sparkles className={`w-5 h-5 ${header.accent} animate-pulse`} />
          </div>
          <div>
            ...
            <p className="text-xs text-white/70 mt-0.5">
              Powered by real-time AegisCare database telemetry & clinical records
            </p>
          </div>
        </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-10 gap-3 text-white/80">
              ...
              <div className="text-sm font-medium animate-pulse">
                Synthesizing real-time telemetry into Gemini executive briefing...
              </div>
            </div>
          ) : error ? (
            <div className="text-xs text-rose-300 bg-rose-950/40 border border-rose-800/40 p-3 rounded-xl">
              {error}
            </div>
          ) : summaryData ? (
                  ...
                  {role === "ADMIN" && (
                    <>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        ...
                        <div className="text-sm font-bold text-emerald-400">₹{(summaryData.metrics.totalRevenueCollected || 0).toLocaleString()}</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                      ...
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        ...
                        <div className="text-sm font-bold text-amber-400">{summaryData.metrics.activeAdmissions} patients</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Registered Patients</div>
                        <div className="text-sm font-bold text-purple-400">{summaryData.metrics.totalRegisteredPatients}</div>
                      </div>
                    </>
                  )}
                  ...
                  {role === "HRMS" && (
                    <>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        ...
                        <div className="text-sm font-bold text-purple-300">₹{(summaryData.metrics.totalMonthlySalaryLiability || 0).toLocaleString()}</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Active Staff Count</div>
                        <div className="text-sm font-bold text-teal-300">{summaryData.metrics.totalActiveEmployees} Employees</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Avg Monthly Salary</div>
                        <div className="text-sm font-bold text-blue-300">₹{(summaryData.metrics.averageMonthlySalary || 0).toLocaleString()}</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Pending Leaves</div>
                        <div className="text-sm font-bold text-amber-400">{summaryData.metrics.pendingLeaveRequestsCount} Requests</div>
                      </div>
                    </>
                  )}
                  ...
                  {role === "FINANCE" && (
                    <>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        ...
                        <div className="text-sm font-bold text-blue-300">₹{(summaryData.metrics.totalBilled || 0).toLocaleString()}</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Revenue Collected</div>
                        <div className="text-sm font-bold text-emerald-400">₹{(summaryData.metrics.totalCollected || 0).toLocaleString()}</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Collection Rate</div>
                        <div className="text-sm font-bold text-teal-300">{summaryData.metrics.collectionRate}%</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Outstanding Receivables</div>
                        <div className="text-sm font-bold text-rose-300">₹{(summaryData.metrics.totalOutstandingReceivables || 0).toLocaleString()}</div>
                      </div>
                    </>
                  )}
                  ...
                  {role === "DOCTOR" && (
                    <>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        ...
                        <div className="text-sm font-bold text-teal-300">{summaryData.metrics.activeInpatientCount} Patients</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Upcoming OPD Consults</div>
                        <div className="text-sm font-bold text-blue-300">{summaryData.metrics.upcomingAppointmentsCount} Tokens</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
                        <div className="text-[10px] text-white/60 font-semibold uppercase">Pending Lab Orders</div>
                        <div className="text-sm font-bold text-amber-300">{summaryData.metrics.pendingLabOrdersCount} Orders</div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-2.5">
